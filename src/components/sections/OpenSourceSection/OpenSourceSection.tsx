...
import type { OpenSourceProductsSectionProps, OpenSourceSectionProps } from './OpenSourceSection.types'
import {
  CategoryDivider,
  CategoryLabel,
  CategorySection,
...
const groupProjectsByCategory = (projects: PortfolioOpenSourceProject[]): PageSection[] => {
  const grouped = new Map<string, PortfolioOpenSourceProject[]>()

  projects.forEach((project) => {
    const category = (project.category ?? DEFAULT_CATEGORY).trim() || DEFAULT_CATEGORY
...
const paginateProjects = (projects: PortfolioOpenSourceProject[]): ProjectPage[] => {
  ...
  groups.forEach(({ category, projects: categoryProjects }) => {
    ...
    while (remaining.length > 0) {
      ...
      if (availableSlots <= 0 || currentSections.length === MAX_CATEGORY_DIVIDERS_PER_PAGE) {
        flushPage()
      }

      const slots = Math.min(MAX_PROJECTS_PER_PAGE - currentCount, remaining.length)
...
const OpenSourceListSection = ({ id, accent, openSource }: OpenSourceListSectionProps) => {
  const pages = useMemo(() => paginateProjects(openSource.projects), [openSource.projects])
  ...
  return (
    <Section id={id} accent={accent} title={openSource.title} description={openSource.caption}>
      {pages.length === 0 ? (
        ...
        <PagesRoot>
          <PagesViewport>
            <PageSlide key={`${pageIndex}-${pages.length}`} $direction={direction}>
              {currentPage.map((section) => (
                <CategorySection key={`${section.category}-${section.projects[0]?.name ?? 'group'}`}>
                  <CategoryDivider>
                    <DividerLine />
                      ...
                      <ProjectCard key={project.name}>
                        ...
                        {project.link && (
                          <ProjectLink
                            ...
                            target={
                              project.link.external || isExternal(project.link.href) ? '_blank' : undefined
                            }
                            rel={
                              project.link.external || isExternal(project.link.href) ? 'noreferrer' : undefined
                            }
                          >
                            {project.link.label}
                          </ProjectLink>
